/**
 * Global Scripts & Lenis Smooth Scroll Setup
 * 
 * Implements desktop-only smooth scrolling using Lenis.
 * Synchronized with GSAP ScrollTrigger.
 * 
 * User Rules:
 * - Desktop only (disabled < 1024px)
 * - Fallback to native on mobile
 * - Disabled on detailed form pages or text-heavy legal pages
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (typeof Lenis !== 'undefined') {
        ...
        function shouldEnableLenis() {
            ...
            // We check for body ID or specific classes
        ...
        function initLenis() {
            ...
            if (enable && !isActive) {
                ...
                lenis = new Lenis({
                    ...
                    smooth: true,
                    mouseMultiplier: 1,
                    smoothTouch: false, // Explicitly disable on touch devices just in case
                    touchMultiplier: 2,
                });
                ...
                // Destroy if active but should not be (e.g. resized down)
                if (lenis) {
                    lenis.destroy();
                    lenis = null;
                }
